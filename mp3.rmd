...
```{r}
sequel_gross_sql <- "
SELECT t.id AS sequel_id, t.production_year AS sequel_year,
	mi.info AS gross
FROM title t
JOIN movie_link ml ON ml.movie_id = t.id
JOIN movie_info mi ON mi.movie_id = t.id
...
"
sequel_gross <- db %>% 
  dbGetQuery(sequel_gross_sql) %>% 
  mutate(gross = gsub(",", "", gross, fixed = TRUE)) %>% 
  mutate(gross = parse_number(gross, na = c("", "NA"), locale = default_locale())) %>% 
...
sequel <- sequel_year %>% 
  left_join(sequel_gross, by = c("sequel_year"))

sequel %>% ggplot(aes(x = sequel_year, y = num_sequel)) + 
  ...
  geom_bar(stat = "identity", aes(fill = avg_gross)) + 
  ...
  scale_fill_continuous(name = "Average Gross\n(Million$)", 
                        limits = c(0, 500000000), 
                        breaks = c(NA, 50000000, 100000000, 500000000), 
                        labels = c("NA", "50", "100", "500"))
  
```


```{r}
inter_median <- sequelinter %>% 
  group_by(sequel_year) %>% 
  summarize(interval_med = median(interval))

sequelinter %>% ggplot(aes(x = sequel_year, y = interval)) + 
  geom_point(alpha = 0.2, position = "jitter") + 
  geom_point(data = inter_median, aes(x = sequel_year, y = interval_med), color = "red4") + 
  geom_line(data = inter_median, aes(x = sequel_year, y = interval_med), color = "red4") + 
